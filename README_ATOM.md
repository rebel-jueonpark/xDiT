...
Failure modes a user on the canonical install (source-built `rebel-compiler`
per [§2.1](#21-build-rebel-compiler-from-source)) may still encounter. Items
that no longer reproduce on this path — `aten::outer` / `aten::rms_norm`
unsupported, `ComposeExpr` arity mismatch, missing `SimplifyTransposeOps`,
generic `DiagnosticError` in PROD — are not listed here; their history is in
[§1](#1-prerequisites) (wheel callout) and [§6](#6-status--roadmap).

| Error / symptom | Cause & fix |
| --- | --- |
| `ImportError: _C.cpython-…so: undefined symbol: _ZN4rbln6pyvmem5debug4FreeEm` (or another `rbln::...` symbol) | The build's `librbln.so` predates the addition of the symbol that `pyrbln/vmem.cc` references — usually after a `git pull` that updated vmem.cc without a rebuild. Re-run `ninja librbln.so` in the rebel-compiler build dir (`source dynamic_linking.env` first so the conan-installed `rbln-ccl` libs are on `LD_LIBRARY_PATH`), then re-do the editable install. Verify with `nm -D build/librbln.so \| grep _ZN4rbln6pyvmem5debug4FreeEm`. |
| `torch._inductor.exc.InductorError` on an `rbln` tensor | Base `_compile_model` correctly dispatches `auto → rbln` on an NPU and `rbln → _compile_transformer_rbln`, but several runner-model overrides still hard-code `torch.compile(..., mode="default")` (Inductor): `runner_models/causal_wan.py`, `runner_models/ltx.py`, `runner_models/hunyuan.py`. Those will fall to Inductor on NPU and fail; fixing requires routing each through `_compile_transformer_rbln`. `flux.py`, `stable_diffusion.py`, and the base path already do the right thing. |
| `RCCL ... failed` / multi-NPU hangs | Make sure the bootstrap env is set (`RCCL_FORCE_EXPORT_MEM=1`, `RCCL_PORT_GEN=1`, `RBLN_ROOT_IP=127.0.0.1`, `RBLN_LOCAL_IP=127.0.0.1`, `MASTER_ADDR=127.0.0.1`) in every rank before `torch_rbln` is imported. Use **tensor parallelism** (`--tensor_parallel_degree N`), not `--ulysses_degree` — `all_to_all` is not compilable into rbln-ccl yet. |
